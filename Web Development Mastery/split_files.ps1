$filePath = "HTML Mastery.html"
$cssPath = "Using Style for HTML Mastery.css"
$jsPath = "Using JS for HTML Mastery.js"
...
for ($i = 0; $i -lt $lines.Length; $i++) {
    ...
    if ($state -eq "HTML" -and $line -match '^\s*<style id="baseStyles">\s*$') {
        $state = "CSS"
        $outHtml += '  <link rel="stylesheet" href="Using Style for HTML Mastery.css">'
        continue
    }
    ...
    elseif ($state -eq "HTML" -and $line -match '^\s*<script>\s*$' -and ($i + 1 -lt $lines.Length) -and $lines[$i+1] -match 'THEME DATA') {
        $state = "JS"
        $outHtml += '  <script src="Using JS for HTML Mastery.js"></script>'
        continue
    }
